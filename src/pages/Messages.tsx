...
const Messages: React.FC = () => {
  ...
  const { messages, loading, loadingMore, hasMore, loadMore, sendMessage, markAsRead } = useMessages();
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    const result = await sendMessage(selectedUser, newMessage);
    if (result.success) {
      setNewMessage('');
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">Communicate with team members</p>
        </div>
        <button
        ...
        <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-hp-blue">{messages.length}</p>
            </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-2xl font-bold text-green-600">
                {new Set([...messages.map(m => m.sender_id), ...messages.map(m => m.receiver_id)]).size}
              </p>
            </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700">
        ...
        {!hasMore && messages.length > 0 && (
          <div className="text-center py-4">
            <p className="text-sm text-gray-500 dark:text-gray-400">No more messages to load</p>
          </div>
        )}
